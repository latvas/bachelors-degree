...
# time of updating the table in seconds
RELOAD_TIME = 5
...
def remove_all_files_in_netlog_dir():
    directory = 'netlog/'
...
def print_pid2traffic():
    ...
    try:
        ...
        printing_df["Download Speed"] = printing_df["Download Speed"].apply(
            get_size).apply(lambda s: f"{s}/s")
        printing_df["Upload Speed"] = printing_df["Upload Speed"].apply(
            get_size).apply(lambda s: f"{s}/s")
    except KeyError as e:
...
def my_print_traffic():
    ...
    for pid, traffic in pid2traffic.items():
        ...
        # get the time the process was spawned
        process = {
            "pid": pid, "name": name, "time": time.time(), "Upload": traffic[0],
            "Download": traffic[1], "process": p, "filename": "netlog/"+str(pid)+name+".csv"
        }
        processes.append(process)

    header_row = tuple("pid time Upload Download".split())
    for process in processes:
        filename = process["filename"]
        line = [process["pid"], time.time(), process["Download"], process["Upload"]]
        print(line)
...
if __name__ == "__main__":
    remove_all_files_in_netlog_dir()
